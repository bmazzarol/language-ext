...
namespace LanguageExt.Next
{
        ...
        IComparable<PositiveDuration>
    {
        ...
        private PositiveDuration(double milliseconds) => _milliseconds = Math.Abs(milliseconds);
        ...
        [Pure]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static implicit operator PositiveDuration(Option<ScheduleResult> result)
            => result.Select(x => (PositiveDuration)x).IfNone(Zero);
